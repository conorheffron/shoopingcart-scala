...
object ProductPriceServiceSpec extends ZIOSpecDefault {
  ...
  override def spec: Spec[TestEnvironment & Scope, Any] = suite("Product Price Service")(
    test("findPriceByProductTitle returns price > 0f for valid products") {
    ...
    test("findPriceByProductTitle Fails for invalid product names") {
    ...
    test("findPriceByProductTitle == 0f for product names that do not have JSON object set up") {
    ...
    test("findPriceByProductTitle == 0f when malformed JSON throws DeserializationException: .title(missing)") {
